...
# -----------------------------------------------------------------------------
# 直接從你已有的 model2.py 裡匯入 SwingCNN (擺放 Feature Dimension 與 Channel)
# -----------------------------------------------------------------------------
...
def init_weights_kaiming(m):
    ...
    對 Conv1d 和 Linear 層使用 Kaiming 正態初始化 (He normal initialization)：
      ...
      - bias: zeros_
    可以透過：model.apply(init_weights_kaiming) 來套用。
    """
...
DATAPATH    = './tabular_data_train'                   # 特徵 CSV 資料夾 (feature CSV folder)
INFO_PATH   = '39_Training_Dataset/train_info.csv'      # train_info.csv 路徑
GROUP_SIZE  = 27                                        # 每個樣本(揮拍段)固定長度 27
TEST_SIZE   = 0.2                                       # 測試集比例 (test split ratio)
RANDOM_SEED = 42                                        # 隨機種子 (random seed)
BATCH_SIZE  = 128                                      # Lightning DataLoader 的 batch size
LR          = 1e-3                                      # 學習率 (learning rate)
MAX_EPOCHS  = 20                                        # 最大訓練 Epoch (max epochs)

# -----------------------------------------------------------------------------
# 2. LightningModule 定義 (改用 CNN 做多類別分類 play years)
# -----------------------------------------------------------------------------
class LitCNNYearsClassifier(pl.LightningModule):
    """
    PyTorch Lightning Module for CNN 多類別分類 (play years)。
    內部使用 SwingCNN，並且在初始化時套用 Kaiming 初始化 (He initialization)。
    """

    def __init__(self, feature_dim: int, num_classes: int, lr: float = 1e-3):
        super(LitCNNYearsClassifier, self).__init__()
        self.save_hyperparameters()  # 將所有 __init__ 的參數存起來，方便 checkpoint/reload

        self.lr = lr
        # 使用 SwingCNN 作為主體
        self.model = SwingCNN(feature_dim=feature_dim, num_classes=num_classes)
        # 套用 Kaiming 初始化 (He initialization)
        self.model.apply(init_weights_kaiming)

        # 損失函數 (loss function)：CrossEntropyLoss 適用於多類別分類
        self.criterion = nn.CrossEntropyLoss()
        # 評估指標 (metrics)：Accuracy，用於 multiclass
    ...
    def _shared_step(self, batch, batch_idx):
        ...
        loss = self.criterion(logits, y)        # 計算 Loss
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 記錄訓練階段的 loss (train_loss) 與 accuracy (train_acc)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # 記錄驗證階段的 loss (val_loss) 與 accuracy (val_acc)
...
# -----------------------------------------------------------------------------
# 3. Callback：蒐集並繪製訓練/驗證 Accuracy 與 Loss 曲線
# -----------------------------------------------------------------------------
class PlotMetricsCallback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        # 取得當前 epoch 的 train_acc、train_loss
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        plt.subplot(1, 2, 1)
        plt.plot(epochs, self.train_accs, label="Train Acc", color="green")
        plt.plot(epochs, self.val_accs,   label="Val Acc",   color="red")
        plt.xlabel("Epochs")
        plt.ylabel("Accuracy")
        ...
        plt.plot(epochs, self.val_losses,   label="Val Loss",   color="orange")
        plt.xlabel("Epochs")
        plt.ylabel("Loss")
...
def evaluate_model(model: nn.Module, dataloader: DataLoader, data_dict, device="cpu"):
    ...
    labels_str = [str(c) for c in data_dict['le_years'].classes_]
...
# -----------------------------------------------------------------------------
# 5. 將「執行訓練、儲存模型、評估」流程包在 if __name__ == '__main__': 裡面
#    以避免 Windows Spawn 問題
# -----------------------------------------------------------------------------
if __name__ == '__main__':
    ...
    # 5.2 取 X / y（play years 多類別）
    X_train = data_dict['X_train']       # (N_train_segments, 27, feature_dim)
    X_test  = data_dict['X_test']        # (N_test_segments, 27, feature_dim)
    y_train = data_dict['y_years']       # (N_train_segments,)
    y_test  = data_dict['y_years_te']    # (N_test_segments,)

    FEATURE_DIM = X_train.shape[2]
    NUM_CLASSES = len(data_dict['le_years'].classes_)

    # 5.3 建 Dataset / DataLoader（注意 num_workers=4、persistent_workers=True）
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        num_workers=4,           # 若仍會出錯，可改成 0 (num_workers=0)
        persistent_workers=True  # Windows 下可加，避免 worker 重複 spawn
    )
    ...
    lit_model = LitCNNYearsClassifier(
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="val_acc",
        dirpath="cnn_years_checkpoints",
        filename="cnn-years-{epoch:02d}-{val_acc:.2f}",
        save_top_k=1,
        mode="max"
    )
    ...
    torch.save(best_model.state_dict(), "cnn_years_model.pth")
    print("Saved PyTorch weights to cnn_years_model.pth")
    ...
    best_model.load_state_dict(torch.load("cnn_years_model.pth"))
